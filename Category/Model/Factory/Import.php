...
class Import extends Factory
{
    ...
    public function setUrlRewrite()
    {
            ...
            if ($connection->tableColumnExists($tmpTable, '_url_rewrite-' . $local)) {
                foreach ($affected as $store) {
                    ...
                    $values = array(
                        ...
                        'entity_id'        => '_entity_id',
                        'prefix'           => new Expr('if(@old_key=`_url_rewrite-' . $local . '`, @old_url_idx:=@old_url_idx+1, @old_url_idx:=0)'),
                        'prev_key'         => new Expr('@old_key:=`_url_rewrite-' . $local . '`'),
                        'request_path'     => new Expr('CONCAT(`_url_rewrite-' . $local . '`, if(@old_url_idx=0,"",CONCAT("-",@old_url_idx)), ".html")'),
                    ...
                    $connection->query('set @old_url_idx =1, @old_key=""');
                    $rewrite = $connection->select()
                        ->from($tmpTable, $values)
                        ->where('`_url_rewrite-' . $local . '` <> ""')
                        ->order('_url_rewrite-' . $local)
                        ->order('_entity_id');
                    $columns = array_diff(array_keys($values), ['prefix', 'prev_key']);
                    $connection->query(
                        $connection->insertFromSelect(
                            $connection->select()->from($rewrite, $columns),
        ...
        $connection->query("
            update $urlRewriteTable as ur1
            join $urlRewriteTable as ur2
             on ur1.entity_type = ur2.entity_type
              and ur1.entity_id = ur2.entity_id
              and ur1.store_id  = ur2.store_id
              and ur1.redirect_type = ur2.redirect_type
              and ur1.is_autogenerated = ur2.is_autogenerated
              and ur1.url_rewrite_id < ur2.url_rewrite_id
            set ur1.is_autogenerated = 0
            where ur1.entity_type = 'category'");
    }
